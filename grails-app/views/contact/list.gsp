...
<html>
    <head>
        ...
        function fAction(act){
          ...
          with( document.forms['frm'] ){
            ...
            if( act == "add" ){
              action = "${createLink(controller: 'contact'  ,action: 'add')}"
            }
            ...
            if( act == "edit" ){
              ...
              action = "${createLink(controller: 'contact'  ,action: 'edit')}"
            }
            
            if( act == "delete" ){
              alert("Disabled")
              return
              action = "${createLink(controller: 'contact'  ,action: 'delete')}"
            }
            
            if( act == "view" ){
              action = "${createLink(controller: 'contact'  ,action: 'view')}"
            }
    ...
    <body>
        ...
        <table id="dt" class="trtbx">
            ...
            <tr onclick="rowSelected(${list[i].contact_id})">
                <td>${list[i].name}</td>
                <td>${list[i].kind}</td>
                <td>${list[i].type}</td>
            </tr>
