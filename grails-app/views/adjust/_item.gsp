  ...
  <table id="tbl_item" title="" style="width:950px;height:auto;">
    <thead>
      <tr>
        ...
        <th style="width:155px;text-align: right" data-options="sortable:true,width:155,field:'4'">Location</th>
        <th style="width:155px;text-align: right" data-options="sortable:true,width:155,field:'5'"></th>
      </tr>
    </thead>
    <tfoot style="display: none">
      <tr>
        ...
        <td style="text-align: right">
          <g:select name='variable_item.location_id' from="${listLocation}"
                    style="width:155px"
    ...
    <tbody>
    <g:each var="item" in="${adjustInstance.items}" status="i">
      <tr id="chidl1_tr_${i}">
        <td style="text-align: left">
        ...
        <td style="text-align: right">
          <g:select name='items[${i}].location_id' from="${listLocation}"
                    style="width:155px" class="easyui-combobox" data-options="disabled:true"
                    optionKey="idname" optionValue="fieldname" value="${item?.location_id}"
          />
        </td>
        <td style="text-align: right">
...
<script type="text/javascript">
    var childCount1 = ${adjustInstance?.items.size()} + 0;
    ...
    function fChangeLocation(){
        //alert(data)
        var lid = $("#location_id").combobox( 'getValue' )
        
        for(x=0; x<childCount1; x++){
          $("#items\\["+x+"\\]\\.location_id").combobox( 'select', lid )
        }
    }
    ...
    function setValues(data){      
      ...
      getUnit(item_id)
      getStock(item_id, $("#location_id").combobox( 'getValue' ))
    ...
    function fSaveChild1() {
      ...
      var ptype   = $("#type").combobox( 'getText' )
      
      if( ptype == "OUT" && qty > unitstk ){
      ...
      if( func == "add" ){
        ...
        $("#items\\["+childCount1+"\\]\\.location_id").combobox(  )
        $("#items\\["+childCount1+"\\]\\.location_id").combobox( {disabled: true} )
        $("#items\\["+childCount1+"\\]\\.location_id").combobox( 'select', $("#location_id").combobox( 'getValue' ) )
