...
<html>
    <head>
        ...
        function fAction(act){
          ...
          with( document.forms['frm'] ){
            ...
            if( act == "add" ){
              action = "${createLink(controller: 'item'  ,action: 'add')}"
            }
            ...
            if( act == "edit" ){
              ...
              action = "${createLink(controller: 'item'  ,action: 'edit')}"
            }
            
            if( act == "delete" ){
              alert("Disabled")
              return
              action = "${createLink(controller: 'item'  ,action: 'delete')}"
            }
            
            if( act == "view" ){
              action = "${createLink(controller: 'item'  ,action: 'view')}"
            }
    ...
    <body>
        ...
        <table id="dt" class="trtbx">
            ...
            <tr onclick="rowSelected(${list[i].item_id})">
                <td>${list[i].name}</td>
                <td>${list[i].kind}</td>
                <td>${list[i].type}</td>
                <td>${list[i].brand}</td>
