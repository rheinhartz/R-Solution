...
<div class="easyui-tabs tab_main">
    <div title="Parent">
      <div style="height:250px">
        ...
        <input type="hidden" name="id" value="${physicalInstance?.id}" />

        <table>
            <tr>
                <td class="${hasErrors(bean:physicalInstance , field:'doc_no','td_error')}">Document No</td>
                <td>: <input type="text" name="doc_no" value="${physicalInstance?.doc_no}"></td>
            </tr>
            <tr>
                <td class="${hasErrors(bean:physicalInstance , field:'location_id','td_error')}">Location</td>
                <td>: 
                  <g:select name='location_id' from="${listLocation}" class="easyui-combobox"
                            style="width:155px"
                            optionKey="idname" optionValue="fieldname" value="${physicalInstance?.location_id}"
            ...
            <tr>
                <td class="${hasErrors(bean:physicalInstance , field:'date','td_error')}">Date</td>
                <td>:
...
<g:if test="${hasErrors(bean:physicalInstance , field:'*','false')}">
<div id="dlg2" class="easyui-dialog" 
     title=" Notice" 
     data-options="closed:false,modal:true,iconCls:'icon-no'"
     style="width:800px;height:400px;padding:10px">
     <div id="error" class="message_error">
        <g:renderErrors bean="${physicalInstance}" as="list" />
     </div>
</div>
</g:if>
